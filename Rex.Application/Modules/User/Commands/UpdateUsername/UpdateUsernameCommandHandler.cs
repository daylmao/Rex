...
) : ICommandHandler<UpdateUsernameCommand, AnswerDto>
{
    public async Task<ResultT<AnswerDto>> Handle(UpdateUsernameCommand request, CancellationToken cancellationToken)
    {
        if (request is null)
        {
        ...
        if (user is null)
        {
            ...
            return ResultT<AnswerDto>.Failure(Error.NotFound("404", $"User with ID {request.UserId} not found."));
        }
        ...
        if (usernameInUse)
        {
            ...
            return ResultT<AnswerDto>.Failure(Error.Conflict("409", $"Username '{request.Username}' is already in use."));
        }
        ...
        return ResultT<AnswerDto>.Success(new AnswerDto("Username updated successfully."));
    }
}